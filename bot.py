...
async def download_file(event, file_name, file_url, progress_message):
    async with semaphore:
        global downloaded_files, failed_files
        try:
            ...
            await progress_message.edit(f"Downloaded {downloaded_files}/{len(lines)} files, Uploaded {uploaded_files}/{len(lines)} files, Failed {failed_files} files")
...
async def handle_docs(event):
    if event.document:
        ...
        download_tasks = [download_file(event, line.strip().split(':', 1)[0], line.strip().plit(':', 1)[1], progress_message) for line in lines]
